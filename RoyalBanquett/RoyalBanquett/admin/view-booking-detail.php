...
$sql= "UPDATE booking SET status=:status,rmark=:remark WHERE id=:eid";
...
// { 
  ...
  $sql="SELECT users.fname,users.mobnumber,users.email,booking.bid,booking.bdate,booking.bfrom,booking.bto,booking.eventtype,booking.nuofguest,booking.msg, booking.rmark,booking.status,booking.updatedate,service.sname,service.sdescription,service.sprice from booking join service on booking.sid=service.id join users on users.id=booking.uid  where booking.id=:eid";
  $query = $dbh->prepare($sql);
  $query-> bindParam(':eid', $eid, PDO::PARAM_STR);
  $query->execute();
  // $results = $query->fetchAll(PDO::FETCH_OBJ);
...
<?php } else { ?>                  <td><?php  echo htmlentities($row['rmark']);?>
                  </td>
                  <?php } ?>
  </tr>
