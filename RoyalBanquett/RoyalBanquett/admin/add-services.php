...
if (strlen($_SESSION['odmsaid'] == 0)) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $sql = "INSERT INTO service (SName, SDescription, SPrice) VALUES (:sername, :serdes, :serprice) RETURNING ID";
        
        $query = $dbh->prepare($sql);
        $query->bindParam(':sername', $sername, PDO::PARAM_STR);
        $query->bindParam(':serdes', $serdes, PDO::PARAM_STR);
        $query->bindParam(':serprice', $serprice, PDO::PARAM_INT);  // Ensure numeric binding
        ...
        if ($query->execute()) {
            ...
            echo '<script>alert("Database Error: Could not insert record")</script>';
        }
    }
}
...
<!doctype html>
 <html lang="en" class="no-focus"> <!--<![endif]-->
    <head>
 <title>RoyalBanquet - Add Services</title>
